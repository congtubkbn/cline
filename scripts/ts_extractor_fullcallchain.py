...
MAX_DEPTH = 10 # Constrained to 3 to prevent exponential explosion; adjust as needed
    ...
    for line in result.stdout.strip().split('\n'):
        ...
        if len(parts) == 3:
            ...
            caller_node = {
                ...
                "callers": trace_callers(node_data.get('name'), current_depth + 1, visited.copy())
            }
            # Deduplicate parallel calls within the same enclosing function
            if not any(c.get('function_name') == caller_node.get('function_name') for c in callers):
...
if __name__ == "__main__":
    ...
    payload["trace_analysis_session"]["total_targets_found"] = len(lines)
